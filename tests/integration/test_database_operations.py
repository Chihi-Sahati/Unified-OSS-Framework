...
class TestDatabaseConnectionPool:
    ...
    async def test_acquire_release_connection(self, db_config):
        ...
        with patch('asyncpg.create_pool') as mock_pool:
            ...
            # Acquire connection
            index, conn = await pool._pool.acquire()
            assert conn is not None
            await pool._pool.release(index)
    ...
    async def test_pool_size_limit(self, db_config):
        ...
        assert pool.config.max_connections == 3
...
class TestTimescaleDBWriter:
    ...
    async def test_insert_kpi_data(self):
        ...
        writer = TimescaleDBWriter(connection_pool=MagicMock())
...
class TestAuditLogger:
    ...
    async def test_log_audit_entry(self):
        ...
        logger = AuditLogger(connection_pool=MagicMock())
