...
class TestAlarmManager:
    ...
    async def test_create_alarm_already_exists(self, alarm_manager, sample_alarm_data):
        ...
        with pytest.raises(AlarmAlreadyExistsError):
            # Force duplicate by setting same alarm_id
            alarm = Alarm(**sample_alarm_data)
            alarm.alarm_id = list(alarm_manager._alarms.keys())[0]
            alarm_manager._alarms[alarm.alarm_id] = alarm
...
class TestAlarmSeverity:
    ...
    def test_severity_ordering(self):
        ...
        assert AlarmSeverity.CRITICAL.value > AlarmSeverity.MAJOR.value
        assert AlarmSeverity.MAJOR.value > AlarmSeverity.MINOR.value
        assert AlarmSeverity.MINOR.value > AlarmSeverity.WARNING.value
