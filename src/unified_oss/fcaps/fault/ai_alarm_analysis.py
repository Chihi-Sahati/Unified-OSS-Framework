...
class AIAlarmAnalyzer:
    ...
    def _extract_features(self, alarm_data: Dict) -> List[float]:
        ...
        # Encode alarm code deterministically
        alarm_code = str(alarm_data.get('alarm_code', ''))
        code_hash = int(hashlib.md5(alarm_code.encode()).hexdigest(), 16)
        features.append((code_hash % 1000) / 1000.0)
        
        # Encode vendor deterministically
        vendor = str(alarm_data.get('vendor', 'unknown')).lower()
        vendor_map = {'ericsson': 0.0, 'huawei': 1.0, 'nokia': 0.3, 'zte': 0.6, 'samsung': 0.8}
        features.append(vendor_map.get(vendor, 0.5))
        
        # Encode element type deterministically
        element_type = str(alarm_data.get('element_type', ''))
        elem_hash = int(hashlib.md5(element_type.encode()).hexdigest(), 16)
        features.append((elem_hash % 100) / 100.0)
        
        # Time features
        now = datetime.utcnow()
        timestamp = alarm_data.get('raised_at', None)
        if timestamp:
            try:
    ...
    def _infer_root_cause(self, model: Any, evidence: Dict) -> str:
        """Infer root cause using Bayesian network inference"""
        try:
            from pgmpy.inference import VariableElimination
            infer = VariableElimination(model)
            
            # Prepare query evidence based on severity counts heuristic
            # Translating severity counts into network evidence bounds
            query_evidence = {}
            if evidence.get('critical_count', 0) > 2:
                query_evidence['node_unreachable'] = 1
            elif evidence.get('major_count', 0) > 3:
                query_evidence['service_degradation'] = 1
            else:
                return 'unknown'

            # Query the probabilities of root causes: power_failure and link_down
            # Compute probabilities given the evidence
            prob_power = infer.query(['power_failure'], evidence=query_evidence).values[1]
            prob_link = infer.query(['link_down'], evidence=query_evidence).values[1]
            
            if prob_power > prob_link and prob_power > 0.5:
...
class AIModelTrainer:
    ...
    def train_correlation_engine(
        ...
        try:
            ...
            # Define specific CPDs based on Domain Knowledge instead of empty models
            cpd_power = TabularCPD(variable='power_failure', variable_card=2, values=[[0.99], [0.01]])
            cpd_link = TabularCPD(variable='link_down', variable_card=2, values=[[0.98], [0.02]])
            
            # P(node_unreachable | power_failure)
            cpd_node = TabularCPD(
                variable='node_unreachable', variable_card=2,
                values=[[0.99, 0.05],   # false
                        [0.01, 0.95]],  # true
                evidence=['power_failure'],
                evidence_card=[2]
            )
            
            # P(service_degradation | link_down, node_unreachable)
            cpd_service = TabularCPD(
                variable='service_degradation', variable_card=2,
                values=[[0.99, 0.40, 0.10, 0.01],   # false
                        [0.01, 0.60, 0.90, 0.99]],  # true
                evidence=['link_down', 'node_unreachable'],
                evidence_card=[2, 2]
            )
            
            model.add_cpds(cpd_power, cpd_link, cpd_node, cpd_service)
            model.check_model()
    ...
    def _prepare_features(self, data: pd.DataFrame) -> np.ndarray:
        ...
        for _, row in data.iterrows():
            ...
            # Time features
            now = datetime.utcnow()
            timestamp = row.get('raised_at', None)
        ...
        if not features_list:
            return np.random.rand(len(data) if len(data) > 0 else 1, 6)
