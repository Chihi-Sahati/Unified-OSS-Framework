...
class CorrelationEngine:
    ...
    async def _correlate_batch(
        ...
        # Group alarms by various criteria
        groups_map: Dict[str, CorrelatedAlarmGroup] = {}
        
        for i, alarm in enumerate(alarms):
            # Check against other alarms
            for j, other in enumerate(alarms):
                ...
                if group:
                    # Check if we already have a group for this
                    if group.group_id not in groups_map:
    ...
    async def _correlate_temporal(
        ...
        alarm_time = datetime.fromisoformat(alarm.get("raised_at", datetime.now(timezone.utc).isoformat()))
        correlated = []
        ...
        for rule in rules:
            ...
            for existing in existing_alarms:
                existing_time = datetime.fromisoformat(
                    existing.get("raised_at", datetime.now(timezone.utc).isoformat())
                )
        ...
        if correlated:
            ...
            group = CorrelatedAlarmGroup(
                group_id=f"temporal-{alarm.get('alarm_id', '')}",
    ...
    async def _correlate_topological(
        ...
        Returns:
            CorrelatedAlarmGroup if correlation found.
        """
        alarm_path = alarm.get("resource_path", "")
        ...
        for existing in existing_alarms:
            existing_path = existing.get("resource_path", "")
        ...
        if correlated:
            ...
            group = CorrelatedAlarmGroup(
                group_id=f"topo-{alarm.get('alarm_id', '')}",
                root_cause_alarm=alarm,
                correlated_alarms=correlated,
    ...
    async def _correlate_causal(
        ...
        alarm_severity = alarm.get("severity", "INDETERMINATE")
        ...
        for pattern in causal_patterns:
            ...
            for existing in existing_alarms:
                existing_text = existing.get("alarm_text", "").lower()
                if any(kw in existing_text for kw in effect_keywords):
                    # Check topological proximity
                    if self._is_topologically_related(
                        self._extract_topology(alarm.get("resource_path", "")),
                        self._extract_topology(existing.get("resource_path", "")),
                    ):
                        correlated.append(existing)
            ...
            if correlated:
                ...
                group = CorrelatedAlarmGroup(
                    group_id=f"causal-{alarm.get('alarm_id', '')}",
    ...
    async def _correlate_cross_vendor(
        ...
        alarm_site = self._extract_topology(alarm.get("resource_path", "")).site_id
        ...
        for existing in existing_alarms:
            ...
            existing_site = self._extract_topology(existing.get("resource_path", "")).site_id
        ...
        if correlated:
            ...
            group = CorrelatedAlarmGroup(
                group_id=f"xvendor-{alarm.get('alarm_id', '')}",
                root_cause_alarm=alarm,
                correlated_alarms=correlated,
    ...
    def calculate_confidence(
        ...
        primary_severity = primary_alarm.get("severity", "INDETERMINATE")
        severity_match = sum(
            1 for a in correlated_alarms
            if a.get("severity") == primary_severity
        ...
        if correlation_type == CorrelationType.TEMPORAL:
            primary_time = datetime.fromisoformat(
                primary_alarm.get("raised_at", datetime.now(timezone.utc).isoformat())
            )
            time_diffs = []
            for a in correlated_alarms:
                a_time = datetime.fromisoformat(
                    a.get("raised_at", datetime.now(timezone.utc).isoformat())
                )
