...
class KPIManager:
    ...
    async def close(self) -> None:
        """Close the KPI manager and clean up resources."""
        logger.info("Closing KPIManager")
        
        # Stop all subscription tasks
        for task_id, task in self._subscription_tasks.items():
            if not task.done():
                logger.debug(f"Cancelling subscription task: {task_id}")
                task.cancel()
        
        if self._subscription_tasks:
            # Wait for tasks to be cancelled
            try:
                # Use a timeout of 1s for safety
                await asyncio.wait(
                ...
                logger.error(f"Error while waiting for subscription tasks to close: {e}")
    ...
    def _get_simulated_counters(self, kpi_def: KPIDefinition) -> Dict[str, float]:
        ...
        counters = {}
        # First pass to generate attempts
        ...
        # Second pass to generate success based on attempts
        for var in kpi_def.vendor_mappings.get(VENDOR_ERICSSON, {}).keys():
            if "success" in var.lower():
                # Try to find matching attempt counter
                # Remove common suffixes and 'success' string to find base name
                clean_name = var.lower().replace("successes", "").replace("success", "").replace("succ", "").strip("_")
                matching_att = None
                for c_name in counters:
                    c_lower = c_name.lower()
                    if ("att" in c_lower or "attempt" in c_lower) and (clean_name in c_lower):
                        matching_att = c_name
                        break
                
                if matching_att and counters[matching_att] > 0:
                    counters[var] = random.randint(int(counters[matching_att] * 0.9), counters[matching_att])
                else:
